...
- **GPU-tier quality scaling:** `detect-gpu` runs at module scope (`src/lib/gpuTier.ts`) and maps GPU tier 0–1 → low, 2 → mid, 3 → high. The `qualityConfig` in Zustand drives all rendering decisions: DPR, shadows, antialias, environment map, particles, decorative GLB loading, point light count, and ShopLight point lights. Defaults to `low` before detection resolves (fail safe) and falls back to `low` on detection failure or if detection exceeds a 2.5s timeout (`detect-gpu` can hang on misbehaving drivers — a `Promise.race` timeout prevents the boot path from stalling).
